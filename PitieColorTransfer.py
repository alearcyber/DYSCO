...
def test():
    ...
    observed, expected = observed.reshape(-1, observed.shape[-1]), expected.reshape(-1, expected.shape[-1])
    out = colour_transfer_idt(observed, expected)
    out = out.reshape(original_shape)
    cv2.imshow("transfered colors", out)
    cv2.waitKey()
